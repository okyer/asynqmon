...
func (h *uiAssetsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Note: Use path.Clean (not filepath) since URL paths always use forward slashes.
	reqPath := path.Clean(r.URL.Path)

	// Get the path relative to the root path.
	if !strings.HasPrefix(reqPath, h.rootPath) {
		http.Error(w, "unexpected path prefix", http.StatusBadRequest)
		return
	}
	reqPath = strings.TrimPrefix(reqPath, h.rootPath)

	if code, err := h.serveFile(w, reqPath); err != nil {
...
func (h *uiAssetsHandler) serveFile(w http.ResponseWriter, filePath string) (code int, err error) {
	...
	filePath = path.Join(h.staticDirPath, filePath)
